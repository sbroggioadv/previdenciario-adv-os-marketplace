...
Funcoes:
- find_cowork(path)       → descobre COWORK root procurando previdenciario/ acima
- hook_state_load/save    → gerencia previdenciario/.hook-state.json (debounce anti-flap)
...
def find_cowork(start: Path) -> Path | None:
    """Procura uma pasta ancestral de `start` que contenha <dir>/previdenciario/cowork-state.json."""
    start = start.resolve()
