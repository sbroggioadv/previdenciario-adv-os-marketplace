...
1. Env var PREVIDENCIARIO_PERSONA aponta para arquivo existente
2. <CWD>/.claude/settings.local.json contem env.PREVIDENCIARIO_PERSONA
...
def _try_env_var() -> Path | None:
    p = os.environ.get("PREVIDENCIARIO_PERSONA")
    if p:
    ...
    for _ in range(5):
        ...
        if settings.exists():
            try:
                ...
                p = env.get("PREVIDENCIARIO_PERSONA")
                if p:
                    path = Path(p).expanduser()
                    if path.exists() and path.is_file():
                        return path
...
def resolve_persona() -> tuple[Path, str]:
    ...
    for fn, tag in (
        (_try_env_var, "env:PREVIDENCIARIO_PERSONA"),
...
def main() -> int:
    ...
    if source == "fallback":
        sys.stderr.write(f"[previdenciario-adv-os] Persona: FALLBACK (nenhuma persona configurada, rode /start)\n")
    else:
